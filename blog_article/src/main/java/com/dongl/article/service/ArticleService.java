package com.dongl.article.service;
...
import com.dongl.article.dao.ArticleDao;
...
public class ArticleService {
	...
	public Article findById(String id) {
	    // 1. 从缓存中获取值
        Article article = (Article) redisUtil.get("article_" + id);
        // 2. 判断是否存在，不存在从数据库中查找。
        if (null == article) {
            article = articleDao.findById(id).get();
            redisUtil.set("article_" + id, article);
        }
		return article;
	}
